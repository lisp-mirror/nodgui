...
added after the last one, and the ids are autogenerated"
    (let* ((res                 (or item
           ...
           (actual-column-values (let ((*suppress-newline-for-tcl-statements* t))
                                   (loop for i in (column-values res) collect
                                        (tclize `(\"+
                                                  ,(tkescape i)
                                                  \"))))))
      ...
      (let ((*suppress-newline-for-tcl-statements*             t)
        ...
        (format-wish (tclize `(,(widget-path object)      " "
                                insert
                                ,(tkescape2 (parent res)) " "
                                ,(wrap-braces index)
                                -id     ,(wrap-braces (id res))
                                ,(tclize-if-true (text res)
                                     `(-text  {+ ,(tkescape (text res)) }))
                                ,(tclize-if-true (image res)
                                     `(-image  ,(wrap-braces (image res))))
                                ,(tclize-if-true (column-values res)
                                     `(-values [list ,actual-column-values ]))
                                ,(tclize-if-true (tag res)
                                     `(-tags   ,(wrap-braces (tag res)))))))
        res)))
